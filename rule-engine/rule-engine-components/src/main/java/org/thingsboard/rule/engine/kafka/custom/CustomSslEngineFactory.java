...
public class CustomSslEngineFactory implements SslEngineFactory {
    ...
    public void configure(Map<String, ?> configs) {
        ...
        this.keystore = createKeystore((String) configs.get(SslConfigs.SSL_KEYSTORE_TYPE_CONFIG),
                ...
                (byte[]) configs.get("ssl.keystore.bytearray"));
        ...
        this.truststore = createTruststore((String) configs.get(SslConfigs.SSL_TRUSTSTORE_TYPE_CONFIG),
                ...
                (byte[]) configs.get("ssl.truststore.bytearray"));
    ...
    protected SecurityStore createKeystore(String type, String path, Password password, Password keyPassword, Password privateKey, Password certificateChain, byte[] keystoreByteArray) {

        if (keystoreByteArray != null) {
        	return new ByteArrayBasedStore(password, keyPassword, keystoreByteArray);
        }
        else {
        	if (privateKey != null) {
    ...
    private static SecurityStore createTruststore(String type, String path, Password password, Password trustStoreCerts, byte[] truststoreByteArray) {
        if (truststoreByteArray != null) {
        	return new ByteArrayBasedStore(password, null, truststoreByteArray);
        }
        else {
    ...
    static class ByteArrayBasedStore implements SecurityStore {
    	
        private final Password password;
        protected final Password keyPassword;
        private final KeyStore keyStore;
		private byte[] keystoreByteArray;
        
        public ByteArrayBasedStore(Password password, Password keyPassword, byte[] keystoreByteArray) {
            this.password = password;
            this.keyPassword = keyPassword;
        ...
        protected KeyStore load() {
        	try {
        		...
        		char[] passwordChars = password != null ? password.value().toCharArray() : null;
        		keyStore.load(new ByteArrayInputStream(keystoreByteArray), passwordChars);
        		return keyStore;
        		
        	}
        	catch (GeneralSecurityException | IOException e) {
                throw new KafkaException("Failed to load SSL keystore", e);
            }
        }
